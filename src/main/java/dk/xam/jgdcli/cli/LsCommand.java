...
@Command(name = "ls", description = "List files in a folder (default: root)", mixinStandardHelpOptions = true)
public class LsCommand implements Callable<Integer> {
    ...
    @Parameters(index = "1", arity = "0..1", description = "Folder ID (default: root)")
    String folderId;
    ...
    @Option(names = "--query", description = "Drive query filter")
    String query;

    @Override
    public Integer call() throws Exception {
        FileListResult result = service.listFiles(email, folderId, query, maxResults, pageToken);
